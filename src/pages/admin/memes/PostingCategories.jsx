...
export default function PostingCategories() {
  ...
  const loadCategories = async (isRefresh = false) => {
    ...
    try {
      const data = await api.get('/admin/memes/posting-categories');
      setCategories(data || []);
    } catch (error) {
  ...
  const handleAdd = async (formData) => {
    try {
      await api.post('/admin/memes/posting-categories', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/admin/memes/posting-categories/${id}`);
  ...
  const handleEditSave = async (id, data) => {
    try {
      await api.patch(`/admin/memes/posting-categories/${id}`, data);
  ...
  const handleToggleActive = async (id, isActive) => {
    try {
      await api.patch(`/admin/memes/posting-categories/${id}`, { isActive });
